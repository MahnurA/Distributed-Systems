...
def onUpdate():
    ...
    for dirpath, dirnames, files in os.walk('/dir/path/'): #replace /dir/path with file path to folder containing other folder(s) and file(s) which you want to include in the single system image
        if not dirnames:
             for file in files:
                 local_list2.append(fileNode(os.path.join(file), "Server_2", "0", "0","0"))
    #print(local_list1)
...
#so reload from its last saved state
        if header == 'crashed\n':
            for i in range(0, len(local_list2)):
                for line in f:
                    line = line.split('$')
                    #print(line[0])
                    #print(local_list2[i].name)
                    if local_list2[i].name == line[0]:
                        local_list2[i].server = line[1]
                        local_list2[i].servercopy = line[2]
                        local_list2[i].replication = line[3]
                        local_list2[i].quarantine = line[4]

#dump files' state and attributes if server crashes
def dump(): 
    global local_list2, directory
    ...
    f.write("crashed" + '\n')   
    for i in range(0,len(local_list2)):
        f.write(local_list2[i].name + '$')
        print(local_list2[i].name)
        f.write(local_list2[i].server + '$')
        f.write(local_list2[i].servercopy + '$')
        f.write(local_list2[i].replication + '$')
        f.write(local_list2[i].quarantine + '$')
        f.write('\n')
...
def ClientThread(clientAddress,clientsocket):
    global local_list1, local_list2, local_list3, Server_1, Server_3 
    ...
    if serverName==b'Server3':
        Server_3 =clientAddress[0]
    print("what2")
    print(Server_1)
    csocket.sendall(bytes("Server2",'UTF-8'))
    if serverName==b'Server1' or serverName==b'Server3':
        try:
            l=[]
        ...
        while True:
            ...
            if serverName==b'Server3':
                gotit = pickle.loads(data)
                local_list3 = gotit
                for x in range(len(local_list3)):
                    print(local_list3[x].name)                
        ...
        while True:
            try:
                ...
                if msg[0:4]=='edit' :
                    ...
                    for i in range(len(local_list2)):
                        if local_list2[i].name==thename:
                            IP=local_list2[i].servercopy
                    if IP=='Server_1':
                        replication(thename, Server_1)
                    if IP=='Server_3':
                        replication(thename, Server_3)
                ...
                if msg[0:6] == 'delete':
                    ...
                    if this_one == "Server_3":
                        csocket.send(bytes(("Connect to"+ Server_3),'UTF-8'))

                if msg[0:9] == 'Repdelete':
                    thename= msg[10:]
                    os.remove("/dir/path/"+thename) #replace with path same as folder included in single system image
                    for i in range(len(local_list2)):
                        if local_list2[i].name==thename and local_list2[i].servercopy== '0':  
                            del local_list2[i]                  
                    csocket.sendall("deleted".encode())
                    for i in range(len(local_list2)):
                        if local_list2[i].name==thename and local_list2[i].servercopy== 'Server_1':  
                            del local_list2[i]                  
                    csocket.sendall("deleted".encode())
                    for i in range(len(local_list2)):
                        if local_list2[i].name==thename and local_list2[i].servercopy== 'Server_3':  
                            del local_list2[i]                  
                    csocket.sendall("deleted".encode())
                ...
                if msg[0:9] == 'Repcreate':
                    ...
                    if clientAddress[0]== Server_1:
                        rep="Server_1"
                    if clientAddress[0]== Server_3:
                        rep="Server_3"
                    local_list1.append(fileNode(thename, "Server_2", rep, "0", "0"))
                    print(thename)
                    csocket.sendall("hi".encode())
                if msg=='bye':
                    break
...
def deletion(thename):   
    global Server_3, local_list1,local_list2,local_list3 , Server_1
    print(thename)
    for i in range(len(local_list2)):
        if local_list2[i].name==thename and local_list2[i].servercopy== '0':
            print("delete 0")
...
def replication(thename, IP=None):
    ...
    if IP==None:
        if len(local_list1)>len(local_list3):
            print("r1")
            rep="Server_1"
            if Server_1=='':
                print("r2")
        ...
        #print(Server_3)
        local_list2.append(fileNode(thename, "Server_2", rep, "0", "0"))
        SERVER=temp
        print("hi")
        print(SERVER)
        print("hi2")
    ...
    if test==3:
        for i in range(len(local_list3)):    
            if local_list3[i].name==fileName:
                c.send(bytes(("Connect to"+ Server_3),'UTF-8'))
                test=0
                return
            else:
                test=1
...
def constantSendingList(cl):
    ...
    while True:
        try :
            csocket=cl
            localdir = pickle.dumps(local_list2)
            csocket.send(localdir)
...
def recUDP():    # Acting like a client to other servers
    ...
    message = b'any one there from Server2?'
...
def initiateTCP(ip):
    ...
    try:
        ...
        client.sendall(bytes("Server2",'UTF-8'))
...
try:
   t2= threading.Thread(target= sendingUDP)
   t2.start()
except:
