...
def onUpdate():
    ...
    for dirpath, dirnames, files in os.walk('/dir/path/'): #replace /dir/path with file path to folder containing other folder(s) and file(s) which you want to include in the single system image
        if not dirnames:
             for file in files:
                 local_list1.append(fileNode(os.path.join(file), "Server_1", "0", "0","0"))
    #print(local_list1)
        ...
        if header == 'crashed\n':
            for i in range(0, len(local_list2)):
                for line in f:
                    line = line.split('$')
                    #print(line[0])
                    #print(local_list1[i].name)
                    if local_list1[i].name == line[0]:
                        local_list1[i].server = line[1]
                        local_list1[i].servercopy = line[2]
                        local_list1[i].replication = line[3]
                        local_list1[i].quarantine = line[4]

#dump files' state and attributes if server crashes
def dump(): 
    global local_list1, directory
    ...
    f.write("crashed" + '\n')    
    for i in range(0,len(local_list1)):
        f.write(local_list1[i].name + '$')
        print(local_list1[i].name)
        f.write(local_list1[i].server + '$')
        f.write(local_list1[i].servercopy + '$')
        f.write(local_list1[i].replication + '$')
        f.write(local_list1[i].quarantine + '$')
        f.write('\n')
...
def ClientThread(clientAddress,clientsocket):
    global local_list1, local_list2, local_list3, Server_2, Server_3 
    ...
    #print("what2")
    print(Server_2)
    csocket.sendall(bytes("Server1",'UTF-8'))
    if serverName==b'Server2' or serverName==b'Server3':
        try:
            l=[]
        ...
        while True:
            try:
                ...
                if msg[0:9] == 'Repdelete':
                    ...
                    for i in range(len(local_list1)):
                        if local_list1[i].name==thename and local_list1[i].servercopy== '0':  
                            del local_list1[i]                  
                    csocket.sendall("deleted".encode())
                    for i in range(len(local_list1)):
                        if local_list1[i].name==thename and local_list1[i].servercopy== 'Server_2':  
                            del local_list1[i]                  
                    csocket.sendall("deleted".encode())
                    for i in range(len(local_list1)):
                        if local_list1[i].name==thename and local_list1[i].servercopy== 'Server_3':  
                            del local_list1[i]                  
                    csocket.sendall("deleted".encode())
                ...
                if msg[0:9] == 'Repcreate':
                    ...
                    if clientAddress[0]== Server_3:
                        rep="Server_3"
                    local_list1.append(fileNode(thename, "Server_1", rep, "0", "0"))
                    print(thename)
...
def deletion(thename):   
    global Server_3, local_list1,local_list2,local_list3 , Server_2
    print(thename)
    for i in range(len(local_list1)):
        if local_list1[i].name==thename and local_list1[i].servercopy== '0':
            print("delete 0")
...
def replication(thename, IP=None):
    ...
    if IP==None:
        if len(local_list2)>len(local_list3):
            rep="Server_2"
            if Server_2=='':
                temp=Server_3
                print(temp)
...
def constantSendingList(cl): 
    ...
    while True:
        try :
            csocket=cl
            localdir = pickle.dumps(local_list1)
            csocket.send(localdir)
...
def recUDP():   # Acting like a client to other servers
    ...
    message = b'any one there from Server1?'
...
def initiateTCP(ip):
    ...
    try:
        ...
        client.sendall(bytes("Server1",'UTF-8'))
